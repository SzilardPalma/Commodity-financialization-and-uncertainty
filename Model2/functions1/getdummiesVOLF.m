function [yd,xd,b0,s0]=getdummiesVOLF(LAMDAPEVIEWS,TAUPEVIEWS,EPSILONPEVIEWS,Y,L,LV,EPSILONPEVIEWS1,outf,outg)
N=cols(Y);
...
[yd,xd] = create_dummies(lamdaP,tauP,deltaP,epsilonP,L,muP,sigmaP,N,LV,EPSILONPEVIEWS1);
% size(yd)
% size(xd)

b0=vec(xd\yd);

  b01=reshape(b0,N*L+(N*LV+1),N);
  e0=yd-xd*b01;
%   S=e0'*e0;
S=diag(sigmaP.^2);
